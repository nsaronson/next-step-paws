...
router.post('/', authenticateToken, bookingValidation, async (req: AuthRequest, res) => {
  try {
    ...
    const slotResult = await pool.query(
      'SELECT * FROM available_slots WHERE id = $1 AND is_booked = FALSE',
      [slotId]
    );
    ...
    try {
      ...
      const bookingResult = await pool.query(
        'INSERT INTO bookings (slot_id, user_id, dog_name, notes, status) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        [slotId, userId, dogName, notes || null, 'confirmed']
      );

      // Mark slot as booked
      await pool.query(
        'UPDATE available_slots SET is_booked = TRUE WHERE id = $1',
        [slotId]
      );
...
router.put('/:id', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const existingBooking = await pool.query(checkQuery, checkParams);
    ...
    if (status === 'cancelled' && currentBooking.status !== 'cancelled') {
      ...
      try {
        ...
        // Free up the slot
        await pool.query(
          'UPDATE available_slots SET is_booked = FALSE WHERE id = $1',
